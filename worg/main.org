...
*  What do I need to know before using the package?
...
** What the package should do in the future but does not yet
...
** Tools from parametric regression and machine learning

At least one of {logistic regression and Cox regression} or {tree based
regression for binary outcome and survival analysis}.
...
#+ATTR_LATEX: :options otherkeywords={}, deletekeywords={}
#+BEGIN_SRC R  :results output raw drawer  :exports results :session *R* :cache yes
Publish::org(skimr::skim(PropScores))
#+END_SRC
...
The clever covariate is a function of the @@latex:{\color{blue}@@desired intervention density@@latex:}@@, the @@latex:{\color{green!70!black}@@ observed intervention densities@@latex:}@@, and the @@latex:{\color{red}@@non-intervention outcome densities@@latex:}@@.
