...
#+BEGIN_SRC R  :results output raw  :exports none  :session *R* :cache no  :eval always
  ...
  counterfactuals <- data.frame(T.j1.a0 = T.j1.a0, T.j1.a1 = T.j1.a1,
				T.j2.a0 = T.j2.a0, T.j2.a1 = T.j2.a1,
				L1 = L1, L2 = L2, L3 = L3)
  observed <- data.frame(T.tilde = T.tilde, Delta = Delta,
...
Under coarsening at random (CAR), the observed data likelihood can be factorized as \[p(O) = p(\X)\, \g(A \mid \X)\, \lambda_c(\T \AX)^{\1(\Delta = 0)} S_c(\T\text{-} \AX) \prod_{j=1}^{J} S(\T\text{-} \AX) \lambda_j(\T \AX)^{\1(\Delta = j)} \] where $\lambda_c(t \AX)$ is the hazard of the censoring process and $\lambda_j(t \AX)$ is the hazard of the $j^{th}$ event process. Additionally \[S_c(t \ax) = \exp\left(-\int_{0}^{t} \lambda_c(s \ax) \,ds\right)\] while in a pure competing risks setting
...
*** Cause-Specific Hazard Estimation
...
For every target event and target time combination, for every person...
